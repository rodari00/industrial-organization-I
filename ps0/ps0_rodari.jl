...
histogram(exp.(log_p),xlabel = L"P_t", ylabel =L"\textrm{Density}", label = "")
#save plot
savefig(string(figures_dir,"\\",ps_n,"_", "hist_p.pdf"))

histogram(exp.(log_q),xlabel = L"Q_t", ylabel =L"\textrm{Density}", label = "")
#save plot
...
gridsize = 10000
grid = linspace(-4,4,gridsize)
ssr_sim = Vector{Float64}(undef,gridsize)
fill!(ssr_sim,NaN) 
...
ssr = Vector{Float64}(undef,gridsize)
fill!(ssr,NaN) 
